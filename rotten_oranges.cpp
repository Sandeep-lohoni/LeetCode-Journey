...
// 2 representing a rotten orange.Every minute,
...
// Input : grid = [ [ 2, 1, 1 ], [ 0, 1, 1 ], [ 1, 0, 1 ] ] Output : -1 Explanation : The orange in the bottom left
// corner(row 2, column 0) is never rotten,
// because rotting only happens 4 - directionally.Example 3 :

// Input : grid = [[ 0, 2 ]] Output : 0 Explanation : Since there are already no fresh oranges at minute 0,
// the answer is just 0.

// Constraints :

// m == grid.length n == grid[i].length 1 <= m,
// n <= 10 grid[i][j] is 0, 1, or 2.
